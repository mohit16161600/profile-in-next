...
/**
 * Server Component. Previously this was a framer-motion client component whose
 * entrance animation server-rendered every block with `style="opacity:0"`, so the
 * hero (including the LCP image) stayed invisible until React hydrated — LCP was
 * 5.9s on throttled mobile. The entrance is now pure CSS from `globals.css`, and
 * the image wrapper animates transform only so the LCP candidate paints on the
 * first frame.
 */
          ...
          <div className="order-2 lg:order-1 text-center lg:text-left">
            ...
            <div className="hero-item hero-item-4 flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <Link href="/#projects" className="px-8 py-4 bg-primary-600 text-white rounded-xl font-semibold shadow-lg shadow-primary-600/25 hover:bg-primary-500 hover:scale-105 hover:shadow-primary-600/40 transition-all duration-300 flex items-center justify-center group">
                View Featured Work
                <svg className="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg>
              </Link>
            ...
            <div className="hero-item hero-item-5 mt-12 flex items-center justify-center lg:justify-start gap-8 text-gray-400 text-sm font-medium">
              ...
              <div className="flex items-center gap-2">
                <span className="p-1 rounded-full bg-yellow-500/20 text-yellow-500"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"></path></svg></span>
                Rated 5/5
              </div>
            </div>
          </div>
